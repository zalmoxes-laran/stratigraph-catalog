...
#
# Who wears it here: the catalogue's own two views (spec §4) — the flat list of
# studies and the HDT view that groups one monument's campaigns over time. NOT
# the reader it also serves: that is EMStudio's built artefact, an Extended
# Matrix tool, and it keeps the EM look.
#
# Why a copy and not a CDN: the Catalog is deployed beside the node, at partners,
# on networks whose route out is somebody else's decision — and a catalogue that
# lost its typeface with the uplink would look broken while working perfectly.
# Same-origin, always. Review the diff, commit it.
#
